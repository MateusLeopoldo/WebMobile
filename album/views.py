...
from album.serializers import SerializadorAlbum
...
class EditarAlbums(LoginRequiredMixin , UpdateView):
    ...
    def get_context_data(self, **kwargs):
        data = super().get_context_data(**kwargs)
        if self.request.POST:
            data['musicas'] = MusicaFormSet(self.request.POST, instance=self.object, prefix='musicas')
        else:
            data['musicas'] = MusicaFormSet(instance=self.object, prefix='musicas')
        return data
...
class APIListarAlbums(ListAPIView):
    '''
